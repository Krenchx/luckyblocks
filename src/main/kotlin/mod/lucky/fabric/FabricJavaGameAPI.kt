...
fun nbtToComponents(tag: CompoundTag, access: HolderLookup.Provider): DataComponentMap {
    try {
        val normalizedTag = tag.copy()
        listOf("Luck" to "lucky:luck", "Drops" to "lucky:drops").forEach { (legacyKey, componentKey) ->
            if (!normalizedTag.contains(componentKey)) {
                normalizedTag.get(legacyKey)?.let { normalizedTag.put(componentKey, it.copy()) }
            }
            normalizedTag.remove(legacyKey)
        }
        val ops = RegistryOps.create(NbtOps.INSTANCE, access)
        return DataComponentMap.CODEC
            .parse(ops, normalizedTag)
            .getOrThrow()
    } catch (e: Exception) {
...
fun componentsToNbt(components: DataComponentMap, access: HolderLookup.Provider): CompoundTag {
    try {
        val ops = RegistryOps.create(NbtOps.INSTANCE, access)
        val encoded = DataComponentMap.CODEC
            .encodeStart(ops, components)
            .getOrThrow()
        val result = encoded as? CompoundTag ?: error("Data components did not encode to a compound tag")
        components.get(DataComponents.CUSTOM_DATA)?.copyTag()?.let { customData ->
            customData.keySet().forEach { key ->
                if (!result.contains(key)) customData.get(key)?.let { result.put(key, it.copy()) }
            }
        }
        return result
    } catch (e: Exception) {
...
object FabricJavaGameAPI : JavaGameAPI {
    ...
    override fun getModVersion(): String {
        return FabricLoader.getInstance().getModContainer("lucky").get().metadata.version.friendlyString
    }

    override fun getMinecraftVersion(): String {
        return FabricLoader.getInstance().getModContainer("minecraft").get().metadata.version.friendlyString
    }
    ...
    override fun showClientMessage(textJsonStr: String) {
        val component = try {
            ComponentSerialization.CODEC.parse(JsonOps.INSTANCE, JsonParser.parseString(textJsonStr))
                .result()
                .orElse(Component.literal(textJsonStr))
        } catch (_: Exception) {
            Component.literal(textJsonStr)
        }
