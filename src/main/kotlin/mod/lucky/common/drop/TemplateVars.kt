...
private val rangeSpec = TemplateVarSpec(
    listOf(
        "minAmount" to ValueSpec(AttrType.INT),
        "maxAmount" to ValueSpec(AttrType.INT),
    ),
    argRange = 0..2,
)

private fun parseRange(templateVar: TemplateVar, defaultRange: IntRange): IntRange {
...
fun registerMultiListTemplateVar(
    ...
    registerTemplateVar(templateName, rangeSpec) { templateVar, context ->
        ListAttr(chooseMultiRandomFrom(context.random, getValues(context), parseRange(templateVar, defaultAmount)))
    }
}

private fun registerMultiDictTemplateVar(
    templateName: String,
    getValues: (context: DropTemplateContext) -> Map<String, ValueAttr>,
    defaultAmount: IntRange = 4..6,
) {
    registerTemplateVar(templateName, rangeSpec) { templateVar, context ->
        val entries = chooseMultiRandomFrom(
            context.random,
            getValues(context).entries.toList(),
            parseRange(templateVar, defaultAmount),
        )
...
fun registerEnchantments(
    ...
    registerMultiDictTemplateVar(
        ...
        getValues = { context ->
            ...
            enchantments.associate { it.id to intAttrOf(context.random.randInt(1..it.maxLevel)) }
        },
...
fun registerCommonTemplateVars(gameType: GameType) {
    ...
    registerTemplateVar("randRGB") { _, context -> intAttrOf(chooseRandomFrom(context.random, GAME_API.getRGBPalette())) }
    ...
    LuckyRegistry.registerTemplateVar("randFireworksRocket") { _, context ->
        val fireworkShape = stringAttrOf(
            chooseRandomFrom(context.random, listOf("small_ball", "large_ball", "star", "creeper", "burst"))
        )
