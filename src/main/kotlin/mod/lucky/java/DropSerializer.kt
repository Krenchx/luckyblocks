...
data class DropContainer(
    ...
    companion object {
        val attrKeys = listOf("lucky:drops", "lucky:luck", "Drops", "Luck")
    }
}
...
fun DropContainer.Companion.fromAttr(attr: DictAttr): DropContainer {
    return DropContainer(
        ((attr["lucky:drops"] ?: attr["Drops"]) as? ListAttr)?.let { dropsFromAttrList(it) },
        attr.getOptionalValue<Int>("lucky:luck") ?: attr.getOptionalValue<Int>("Luck"),
    )
}
